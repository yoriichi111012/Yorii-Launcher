...
public final class AuthlibInjector {
	...
	public static synchronized void bootstrap(Instrumentation instrumentation, String apiUrl) throws InitializationException {
		...
		boolean skinOnlyMode = (apiUrl == null || apiUrl.isEmpty());
		APIMetadata apiMetadata;
		if (skinOnlyMode) {
			log(INFO, "Skin-only mode: no authentication server");
			apiMetadata = createSkinOnlyMetadata();
		} else {
			apiMetadata = fetchAPIMetadata(apiUrl);
		}

		classTransformer = createTransformer(apiMetadata, skinOnlyMode);
		...
		if (!Config.noShowServerName && !skinOnlyMode) {
			AuthServerNameInjector.init(apiMetadata);
		}
	}

	private static APIMetadata createSkinOnlyMetadata() {
		List<String> skinDomains = new ArrayList<>();
		skinDomains.add("raw.githubusercontent.com");
		Map<String, Object> meta = new TreeMap<>();
		meta.put("feature.legacy_skin_api", Boolean.TRUE);
		return new APIMetadata("", skinDomains, meta, Optional.empty());
	}
	...
	private static List<URLFilter> createFilters(APIMetadata config, boolean skinOnlyMode) {
		...
		if (skinOnlyMode) {
			filters.add(new LegacySkinAPIFilter(null, Config.githubSkinIndexUrl));
			return filters;
		}
		...
		if (Config.legacySkinPolyfill.isEnabled(legacySkinPolyfillDefault)) {
			filters.add(new LegacySkinAPIFilter(customClient, Config.githubSkinIndexUrl));
		} else {
	...
	private static ClassTransformer createTransformer(APIMetadata config, boolean skinOnlyMode) {
		...
		URLProcessor urlProcessor = new URLProcessor(createFilters(config, false), new DefaultURLRedirector(config));
