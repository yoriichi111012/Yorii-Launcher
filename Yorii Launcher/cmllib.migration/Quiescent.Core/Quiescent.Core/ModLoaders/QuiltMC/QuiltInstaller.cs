...
namespace Quiescent.Core.ModLoaders.QuiltMC;
...
public class QuiltInstaller
{
    ...
    public async Task<IReadOnlyCollection<string>> GetSupportedVersionNames()
    {
        ...
        var list = await Quiescent.Core.Commons.Serialization.QuiescentJson.DeserializeAsync<IEnumerable<QuiltLoader>>(res);
        if (list == null)
    ...
    public async Task<IReadOnlyCollection<QuiltLoader>> GetLoaders()
    {
        ...
        var list = await Quiescent.Core.Commons.Serialization.QuiescentJson.DeserializeAsync<IReadOnlyCollection<QuiltLoader>>(res);
        if (list == null)
    ...
    private IEnumerable<QuiltLoader> parseLoaders(JsonElement root)
    {
        ...
        foreach (var item in root.EnumerateArray())
        {
            if (item.ValueKind != JsonValueKind.Object)
                continue;
            var loader = Quiescent.Core.Commons.Serialization.QuiescentJson.Deserialize<QuiltLoader>(item.GetPropertyOrNull("loader"));
            if (loader != null)
                yield return loader;
        }
    }
