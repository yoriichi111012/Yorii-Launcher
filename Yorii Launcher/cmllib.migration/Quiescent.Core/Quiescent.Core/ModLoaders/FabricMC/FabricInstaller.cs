...
namespace Quiescent.Core.ModLoaders.FabricMC;
...
public class FabricInstaller
{
    ...
    public async Task<IReadOnlyCollection<string>> GetSupportedVersionNames()
    {
        ...
        var list = await Quiescent.Core.Commons.Serialization.QuiescentJson.DeserializeAsync<IEnumerable<FabricLoader>>(res);
        if (list == null)
    ...
    public async Task<IReadOnlyCollection<FabricLoader>> GetLoaders()
    {
        ...
        var list = await Quiescent.Core.Commons.Serialization.QuiescentJson.DeserializeAsync<IReadOnlyCollection<FabricLoader>>(res);
        if (list == null)
    ...
    private IEnumerable<FabricLoader> parseLoaders(JsonElement root)
    {
        ...
        foreach (var item in root.EnumerateArray())
        {
            if (item.ValueKind != JsonValueKind.Object)
                continue;
            var loader = Quiescent.Core.Commons.Serialization.QuiescentJson.Deserialize<FabricLoader>(item.GetPropertyOrNull("loader"));
            if (loader != null)
                yield return loader;
        }
    }
