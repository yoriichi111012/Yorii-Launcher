...
public class JsonVersion : IVersion, IDisposable
{
    ...
    public JsonVersion(JsonDocument jsonDocument, JsonVersionParserOptions options)
    {
        _options = options;
        _json = jsonDocument;
        _model = Quiescent.Core.Commons.Serialization.QuiescentJson.Deserialize<JsonVersionDTO>(jsonDocument.RootElement) ?? 
    ...
    private MFileMetadata? getClient()
    {
        try
        {
            return Quiescent.Core.Commons.Serialization.QuiescentJson.Deserialize<MFileMetadata>(_json.RootElement
    ...
    private MLogFileMetadata? getLogging()
    {
        try
        {
            return Quiescent.Core.Commons.Serialization.QuiescentJson.Deserialize<MLogFileMetadata>(_json.RootElement
                .GetProperty("logging")
                .GetProperty(_options.Side));
        }
        catch (KeyNotFoundException)
