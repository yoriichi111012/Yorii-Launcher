...
public static class JsonLibraryParser
{
    public static MLibrary? Parse(JsonElement element)
    {
        ...
        if (artifactProp.HasValue)
            artifact = Quiescent.Core.Commons.Serialization.QuiescentJson.Deserialize<MFileMetadata>(artifactProp.Value);
        ...
        if (classifiersProp.HasValue)
            classifiers = Quiescent.Core.Commons.Serialization.QuiescentJson.Deserialize<Dictionary<string, MFileMetadata>>(classifiersProp.Value);

        // natives
        IReadOnlyDictionary<string, string>? natives = null;
        var nativesProp = element.GetPropertyOrNull("natives");
        if (nativesProp.HasValue)
            natives = Quiescent.Core.Commons.Serialization.QuiescentJson.Deserialize<Dictionary<string, string>>(nativesProp.Value);
        ...
        {
            artifact = Quiescent.Core.Commons.Serialization.QuiescentJson.Deserialize<MFileMetadata>(element);
        }
