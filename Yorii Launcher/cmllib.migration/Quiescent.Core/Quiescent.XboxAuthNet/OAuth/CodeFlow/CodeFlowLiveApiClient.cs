...
public class CodeFlowLiveApiClient : ICodeFlowApiClient
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            string reqBody = "(none)";
            try { if (req.Content != null) reqBody = await req.Content.ReadAsStringAsync(cancellationToken).ConfigureAwait(false); } catch { }
            var reqQuery = req.RequestUri?.Query;
            throw new OAuth.DiagnosticOAuthException(
                $"[OAUTH] {statusCode}: {reasonPhrase} | url: {req.RequestUri} | reqBody: {(string.IsNullOrEmpty(reqBody) ? "(empty)" : reqBody.Length <= 300 ? reqBody : reqBody[..300] + "...")} | respBody: {(string.IsNullOrEmpty(resBody) ? "(empty)" : resBody.Length <= 300 ? resBody : resBody[..300] + "...")}",
                statusCode, reqQuery);
        }
