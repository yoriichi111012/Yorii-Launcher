...
namespace Quiescent.Core.Auth.Microsoft.Sessions;
    ...
    public static JEProfile ParseFromJson(JsonElement root)
    {
        ...
        try
        {
            profile.Skins = root
                .GetProperty("skins")
                .EnumerateArray()
                .Select(arr => Quiescent.Core.Commons.Serialization.QuiescentJson.Deserialize<JEProfileSkin>(arr))
        ...
        try
        {
            profile.Capes = root
                .GetProperty("capes")
                .EnumerateArray()
                .Select(arr => Quiescent.Core.Commons.Serialization.QuiescentJson.Deserialize<JEProfileCape>(arr))
