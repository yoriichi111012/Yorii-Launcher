...
public class JsonSessionStorage : ISessionStorage
{
    ...
    private bool getFromJson<T>(string key, out T value)
    {
        ...
        {
            var jsonValue = Quiescent.Core.Commons.Serialization.QuiescentJson.Deserialize<T>(_jsonObject[key]!);
            if (jsonValue == null)
            {
                value = default!;
                return false;
    ...
    public JsonObject ToJsonObjectByKeys(IEnumerable<string> keys)
    {
        ...
        foreach (var key in keys)
        {
            ...
            if (!result)
                continue;
            var node = Quiescent.Core.Commons.Serialization.QuiescentJson.SerializeToNode(value);
            json.Add(key, node);
        }
        return json;
    }
